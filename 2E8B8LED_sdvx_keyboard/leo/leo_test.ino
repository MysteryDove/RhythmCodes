...
unsigned long lastEncoderTime0 = 0;
...
void loop() {
  ...
  if (loopTime < 1000) {
    delayMicroseconds(2000 - loopTime);
  }
  //ReportRate Display
...
void doEncoder0() {
  // Debounce check
  if (millis() - lastEncoderTime0 < encoderDebounceTime) {
    return;
  }
  lastEncoderTime0 = millis();
  
  // Rest of your encoder code
...
void doEncoder1() {
  // Debounce check
  if (millis() - lastEncoderTime1 < encoderDebounceTime) {
    return;
  }
  lastEncoderTime1 = millis();
